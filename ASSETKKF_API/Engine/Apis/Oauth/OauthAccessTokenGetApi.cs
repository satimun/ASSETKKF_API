...
namespace ASSETKKF_API.Engine.Apis.Oauth
{
    public class OauthAccessTokenGetApi : Base<dynamic>
    {
        ...
        protected override void ExecuteChild(dynamic dataReq, ResponseAPI dataRes)
        {
            ...
            var res = ASSETKKF_ADO.Mssql.Mcis.muAccessTokenAdo.GetInstant().Search(AccessToken);

            if (res.Count == 1)
            {
                ASSETKKF_ADO.Mssql.Mcis.muAccessTokenAdo.GetInstant().Update(this.AccessToken);
            }
            else
            {
                this.AccessToken = Core.Util.EncryptUtil.NewID(this.IPAddress);
                ASSETKKF_ADO.Mssql.Mcis.muAccessTokenAdo.GetInstant().Insert(this.AccessToken, this.IPAddress, this.UserAgent);
            }
