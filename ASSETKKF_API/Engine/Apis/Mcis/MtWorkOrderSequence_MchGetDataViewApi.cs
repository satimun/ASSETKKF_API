...
{
    public class MtWorkOrderSequence_MchGetDataViewApi : Base<MtWorkOrderSequence_MchReq>
    {
        public MtWorkOrderSequence_MchGetDataViewApi()
        {
            PermissionKey = "ADMIN";
        ...
        protected override void ExecuteChild(MtWorkOrderSequence_MchReq dataReq, ResponseAPI dataRes)
        {
            ...
            try
            {
                var roles = ASSETKKF_ADO.Mssql.Mcis.MtWorkOrderSequence_MchAdo.GetInstant().GetDataView(dataReq);
                ...
                if (roles.Count == 0)
                {
                    ...
                    tmp._result._message = "ไม่พบข้อมูล ปิดงานใบสั่งผลิต.(เครื่อง)" + dataReq.WorkOrderId;
                ...
                else
                {
                    foreach (var x in roles)
                    {
                        ...
                        tmp.User_dateStr = DateTimeUtil.ToStringViewDT(x.User_date);
                        tmp.WantdateStr = DateTimeUtil.ToStringViewDT(x.WorkDate);
