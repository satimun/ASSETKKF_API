...
{
    public class MtWorkOrderSequence_EmpGetDataViewApi : Base<MtWorkOrderSequence_EmpReq>
    {
        public MtWorkOrderSequence_EmpGetDataViewApi()
        {
            PermissionKey = "ADMIN";
        ...
        protected override void ExecuteChild(MtWorkOrderSequence_EmpReq dataReq, ResponseAPI dataRes)
        {
            ...
            try
            {
                var roles = ASSETKKF_ADO.Mssql.Mcis.MtWorkOrderSequence_EmpAdo.GetInstant().GetDataViwe(dataReq);
                ...
                if (roles.Count == 0)
                {
                    ...
                    tmp._result._message = "ไม่พบข้อมูล ปิดงานใบสั่งผลิต.(คน)" + dataReq.WorkOrderId;
