     ...
     public class AuditAccSendApi : Base<AuditPostReq>
    {
        ...
        private AuditAccRes sendMst(AuditPostReq dataReq, AuditAccRes res)
        {
            try
            {
                ...
                var task1 = System.Threading.Tasks.Task.Factory.StartNew(() => ASSETKKF_ADO.Mssql.Audit.AUDITPOSTMSTTODEPAdo.GetInstant().SP_AUDITPOSTMSTTODEPPHONE(req));
                task1.Wait();
            }
            catch (Exception ex)
            {
        ...
        private AuditAccRes sendTrn(AuditPostReq dataReq, AuditAccRes res)
        {
            try
            {
                ...
                var task2 = System.Threading.Tasks.Task.Factory.StartNew(() => ASSETKKF_ADO.Mssql.Audit.AUDITPOSTMSTTODEPAdo.GetInstant().SP_AUDITPOSTMSTTODEPPHONE(req1));
                task2.Wait();
            }
            catch (Exception ex)
            {
        ...
        private AuditAccRes save(AuditPostReq dataReq, AuditAccRes res)
        {
            ...
            finally
            {
                ...
                var lst = ASSETKKF_ADO.Mssql.Audit.AUDITPOSTMSTTODEPAdo.GetInstant().getDataToACCEdit(req);
                res.AUDITPOSTMSTTODEPLST = lst;

                var lstPostTRN = ASSETKKF_ADO.Mssql.Audit.AUDITPOSTTRNAdo.GetInstant().getPOSTTRNACC(req);
                res.POSTTRNDuplicateLST = lstPostTRN;


                var summaryAudit = AuditAccAdo.GetInstant().GetSummaryAudit(dataReq);
                res.AuditSummary = summaryAudit;

                var lstResult = AuditAccAdo.GetInstant().GetSummaryResult(dataReq);
